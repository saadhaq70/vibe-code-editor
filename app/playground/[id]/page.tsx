...
const MainPlaygroundPage = () => {
  ...
  // Define activeFile here BEFORE using it in hooks
  ...
  // Apply AI modifications to files
  const handleApplyModifications = useCallback(
    async (modifications: FileModification[]) => {
      if (!templateData) {
      ...
      try {
          ...
          for (let i = 0; i < items.length; i++) {
            ...
            if ("folderName" in item) {
              ...
              if (targetPath.endsWith(itemPath) || targetPath === itemPath) {
                items[i] = { ...item, content: newContent };
        ...
        for (const mod of modifications) {
          if (mod.changeType === "update") {
            const updated = updateFileInStructure(
            ...
            if (!updated) {
              console.warn(`⚠ File not found: ${mod.filePath}`);
            }

            // Also update in WebContainer if available
            ...
            // Handle file creation (add to template structure)
            console.log(`Creating new file: ${mod.filePath}`);
            // For now, log it - full implementation would add to structure
          }
        }
        ...
        const updatedOpenFiles = openFiles.map((file) => {
          ...
          const modification = modifications.find((m) =>
            m.filePath.endsWith(filePath) || m.filePath === filePath
          );
          ...
          if (modification) {
            return {
              ...
              originalContent: modification.modifiedContent,
        ...
        toast.success(
          `Applied ${modifications.length} modification(s) successfully`
        );
            ...
            <div className="flex flex-1 items-center gap-2">
              ...
              <div className="flex items-center gap-1">
               ...
               <ToggleAI
                ...
                currentFile={activeFile ? `${activeFile.filename}.${activeFile.fileExtension}` : undefined}
                onApplyModification={async (modification) => {
                  await handleApplyModifications([modification]);
                }}
               />
